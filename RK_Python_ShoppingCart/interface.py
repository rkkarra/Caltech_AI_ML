...
class Interface:
    ...
    def user_interface(self, user, username):
        ...
        print(" - Proceed to checkout (Enter C) ")
        ...
        if user_command == "S":
            ...
            for k, v in self.products.items():
                if k == cat:
                    for i in v:
                        if i.product_name == nm and i.quantity > qty > 0:
                            user.cart.update(nm, qty)
                            print(f"You've updated {i.product_name} with new quantity {qty}")
        elif user_command == "R":
            cat = input("Category: ")
            nm = input("Product: ")
            for k, v in self.products.items():
                if k == cat:
                    for i in v:
                        if i.product_name == nm:
                            user.cart.remove(nm)
                            print(f"You've removed {i.product_name} from your cart")
        elif user_command == "C":
            user.cart.view()
            print("This is your cart with total.")
            if input("Proceed to payment? (Y / N) ") == "Y":
                payment_method = input("Which payment method?")
                print(" - PayPal (P) ")
                print(" - Square (S) ")
                if payment_method == "P":
                    print("Redirecting to www.paypal.com")
                elif payment_method == "S":
                    print("Redirecting www.square.com")
        elif user_command == "E":
            exit()
